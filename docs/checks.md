...
| `split_compatibility` | Unseen test classes/categories, changed numeric parsing, increased missingness | Both splits | Type/category signals require minimum training samples; nonnumeric training cardinality <=50 for category checks; does not inspect encoders |
...
Compatibility uses `missing_threshold` for the absolute increase in missing
fraction between splits (0.2 means 20 percentage points). Unseen feature
categories need context; unknown-category handling may already be intentional.
