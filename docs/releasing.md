...
python -m build --outdir artifacts/release-0.6.0
python -m twine check --strict artifacts/release-0.6.0/*
python scripts/check_distribution.py artifacts/release-0.6.0
```
...
3. Verify from a fresh environment:
   `python -m pip install --index-url https://test.pypi.org/simple/ --no-deps proofml==0.6.0`.
4. Create/push the reviewed `v0.6.0` tag. Dispatch the same workflow from that
   tag with destination `pypi`. The workflow checks that tag and version match.
5. Verify with `python -m pip install --index-url https://pypi.org/simple/ proofml==0.6.0`,
